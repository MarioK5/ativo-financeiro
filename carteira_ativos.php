...
function busca_ativos($idCliente)   {
	...
	if (mysqli_num_rows($result) > 0) {
		...
		while ($row = mysqli_fetch_array($result)) {
            		...
            		$descricao  = $row["DESCRICAO"];
            		$idCliente  = $row["ID_CLIENTE"];
			$dados_array =($idCarteira.';'.$descricao);
		...
		$tela .= '<div class="row">
					...
					<tr style="color:white; background-color:#2F4F4F;">
							...
							<button type="button" class="btn btn-default btn-xs" onclick="xajax_editar_ativo_carteira('.$dados_array.');">
...
function editar_ativo_carteira($dados)   {
